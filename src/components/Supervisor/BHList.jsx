...
import { SearchOutlined, UserOutlined, IdcardOutlined, ClockCircleOutlined, HourglassOutlined, ReloadOutlined } from '@ant-design/icons'
import { HappyProvider } from '@ant-design/happy-work-theme'

const BHList = ({identificador, id_consulta, nomina}) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
        ...
        try {
            ...
            const url_get = "http://localhost/wl-api/BhConsultList.php"
  ...
  const dataSource = data.map((jsonres, index) => ({
    ...
    actual: jsonres.s_actual,
    faltantes: jsonres.horas_faltantes
  }))
  ...
  const columns = [
    ...
    },
    {
      title: (
        <Space size={4} className="text-slate-700">
          <HourglassOutlined className="text-purple-500" />
          <span className="font-semibold">Horas Faltantes</span>
        </Space>
      ),
      dataIndex: 'faltantes',
      key: 'faltantes',
      width: 150,
      align: 'center',
      sorter: (a, b) => a.faltantes - b.faltantes,
      render: (text) => {
        const value = parseFloat(text) || 0;
    ...
    <div className="bg-white rounded-xl shadow-md border border-slate-200 overflow-hidden">
      ...
      <div className="px-4 py-2 bg-slate-50 border-t border-slate-100 text-xs text-slate-500 flex justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="flex items-center gap-1">
            <span className="w-3 h-3 bg-rose-100 rounded-full"></span>
            Pendiente
          </span>
        </div>
      </div>
